...
多线程共享全局变量，出现的问题：
    累加次数不够
产生原因：
    线程1还没有来得及执行完（一个完整的动作）前，被线程2抢走了资源，就可能出现问题
解决方案：
    加锁思想，即：互斥锁
细节：
    使用互斥锁的时候，要在合适的时机释放锁，否则可能出现死锁或锁不住的情况
        1.死锁：程序阻塞，但是没有终止
        2.锁不住：不止一把锁
进程和线程的区别：
        1.线程依赖进程，进程是CPU分配资源的基本单位，线程是CPU调度资源的基本单位
        2.进程更消耗资源，不能共享全局变量，相对更稳定
        3.线程更轻量级，可以共享全局变量，相对灵活
"""
...
#         global_num += 1
#     print(f'线程1执行完毕，global_num的值为：{global_num}')
#     mutex.release()
# def target_fun2():
#     mutex.acquire()
#     # mutex1.acquire()
...
#     for i in range(1000000):
#         global_num += 1
#     print(f'线程2执行完毕，global_num的值为：{global_num}')
#     mutex.release()
#     # mutex1.release()
#
