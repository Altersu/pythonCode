...
import re
email = "_a11bd@163.com"
result = re.match(r'^[a-zA-Z0-9_]{4,20}@(163|qq|126)\.com$',email)
if result:
    print(f'合法邮箱为: {result.group()}')
    print(f'合法邮箱为: {result.group(0)}')  # 获取第0组的信息, 效果同上, 即: 获取整个匹配到的结果.
    print(f'合法邮箱为: {result.group(1)}')  # 获取第1组信息, 即: 163
else:
    print("邮箱不合法")